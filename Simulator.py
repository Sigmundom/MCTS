...
class ManualPlayer(Player):
  def get_next_move(self, state: State, possible_actions:PossibleActions):
    action = inquirer.number(
      ...
      invalid_message='You must choose a number between {} and {}'.format(possible_actions[0], possible_actions[-1]),
      default=None
      ).execute()
    return int(action)
  ...
  def run(self):
    ...
    while not board.is_terminal_state:
      ...
      if board._players_turn:
        next_move = self.player1.get_next_move(board.state, board.get_possible_actions())
      else:
        next_move = self.player2.get_next_move(board.state, board.get_possible_actions())
