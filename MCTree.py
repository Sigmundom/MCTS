...
class MCNode:
  ...
  def tree_policy(self) -> int:
    ...
    if self._players_turn:
      best_action = max(shuffled_children, key=lambda x: x[1].expected_final_result + x[1].exploration_bonus)[0]
    else:
      best_action = min(shuffled_children, key=lambda x: x[1].expected_final_result - x[1].exploration_bonus)[0]
    return best_action
...
class MCTree:
  ...
  def simulate(self, board: StateManager, default_policy: Callable[[State, PossibleActions], Action]):
    ...
    if not board.is_terminal_state:
      ...
      next_action = choice(possible_actions)
      current_node = current_node.children[next_action]
      board.perform_action(next_action)
