...
def get_neighbours(r, c, dim):
    potensial_neighbours = [(r-1, c), (r-1, c+1),
                            (r, c-1), (r, c+1), (r+1, c-1), (r+1, c)]
...
class Hex(StateManager):
    ...
    def one_hot_encode(cls, state) -> np.array:
        one_hot = np.empty(2*len(state)-1)
        one_hot[0] = state[0]
        for i in range(len(state)-1):
            one_hot[2*i+1] = 1 if state[i+1] == 1 else 0
            one_hot[2*i+2] = 1 if state[i+1] == 2 else 0
        return one_hot
